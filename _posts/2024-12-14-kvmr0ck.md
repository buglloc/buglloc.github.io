...
categories:
  ...
  - home-infra
---
В этом посте речь пойдет о сборке и установке PiKVM на [Radxa ZERO 3W](http://radxa.com/products/zeros/zero3w/) с [HDMI-CSI bridge C790](https://a.aliexpress.com/_olvAXLh). Должен получится "топ за свои деньги", а получился ли узнаем в прямом сравнении :)
...
| Graphics   | Arm Mali‑G52‑2EE                                                                   | Broadcom VideoCore IV                                                                        |
...
Конкретно у меня Radxa Zero 3W с 4GB RAM и 32GB eMMC.

По деньгам получилось около $70:
  ...
  - три Type-C USB коннектора и макетная платка - около $2-3
  - какой-то корпус, который нашел в столе - пусть будет еще $2

Я бы сказал, что-то среднее между бомж-вариантом с HDMI-USB донглом и люксовым [PiKVM V3](https://a.aliexpress.com/_oDD5by7)/[TinyPilot Voyager 2a](https://tinypilotkvm.com/product/tinypilot-voyager2a)
...
Осталось дело за мылым - запаять и подсобрать:
...
Вместо [PiKVM](https://pikvm.org/) можно было бы взять [TinyPilot](https://github.com/tiny-pilot/tinypilot), но я не нашел для себя преимуществ последнего (кроме желания денег за поддержку HTTPS офк). Ну да хватит болтовни. Cтавим [Debian 12 (Bookworm) Minimal](https://www.armbian.com/radxa-zero-3/) на eMMC, делаем первичную настройку (WiFi, SSH, вот это все), топаем по SSH и...
...
```
root@kvmr0ck:~# vim /etc/kvmd/override.yaml 
root@kvmr0ck:~# cat /etc/kvmd/override.yaml 
kvmd:
	hid:
		mouse_alt:
...
## Closure
Получился ли "топ за свои деньги", пока не сделаешь прямое сравнение, не поймешь. Зато что точно можно сказать - оно работает, работает достаточно хорошо чтобы делать траблшутинг любой сложности. Да, есть заметный input lag и в игрушки не поиграешь, но так ли это важно для типичного ops и при цене _одного_ вечера в пабе?
Вот что расстроило чуть больше - CSI разъем у Radxa Zero 3W какой-то хлипкий и нуждается в улучшении, может доберусь до колхозинга и вы прочтете еще один пост. Кто знает ;)
