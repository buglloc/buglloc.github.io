...
Я выбрал PR для ESPHome в надежде что его допинают до релиза и он станет доступен из коробки. Кмк, полезнее был бы порт RadioLib, но и CC1101 в составе ESPHome тоже не плохо :)
...
Кстати, помните я говорил что памяти у nRF52810 впритирку для BLE стека Zephyr? Вот какой-никакой пруф со сборкой PinkyWinky (тут отлючено логирование, serial console, не используемые фичи bluetooth и пр.):
```
...
Остаются сущие мелочи. Припаяться к test poins на платке:
