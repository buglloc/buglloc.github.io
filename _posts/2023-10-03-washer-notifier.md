...
Так уж вышло, что в один солнечный день я осознал, что больше жить не могу без нотификаций от стиральной машины. Все дело в том, что в текущей квартире она стоит на балконе и о ней, как очень просто забыть, так и не услышать вовсе. Как итог, забытые вещи и горе в семье.
![](/assets/images/posts/wn/cover.png){: .align-center}

### User story
Слышал, что без user story обречен почти любой проект, поэтому я не стал рисковать и представил идеальный flow для себя:

![](/assets/images/posts/wn/the-plan.excalidraw.png){: .align-center}

Ага, ок, понятно, звучит так, что нам нужно:
...
### WRS
Немного гуглежа и список вариантов решений (скорее всего не полный) был у меня на столе. И так, можно было сделать:
...
- по-богатому как-то слишком уж бессмысленно и беспощадно для съемного кондоминиума
- рукастый варик отъехал по этой же причине - техника не моя + я довольно рукожопый
- sound recognition хоть и звучит не плохо (кек ^^), но профит выглядит мелковатым для потраченных усилий
...
Остались два варика, которые я решил реализовать одновременно и сравнить. Приступим!

#### WRS: vibration sensor
Начать я, конечно же, решил со всратого. Ничего не могу с собой поделать, всратости попадают мне в самое сердечко :)

На всякий случай, концепция очень проста:
...
Подключил сию не хитрую конструкцию:
```markdown
| ESP32 | SW-18010P |
|-------|-----------|
|  3.3V |     +     |
...
sensor:
  ...
  - platform: pulse_counter
    ...
    filters:
    - median:
        window_size: 60 # скользяще окно около минуты
...
Мне очень не нравятся оба графика, но тот который на основе вибраций не нравится сильно больше. Поэтому решил продолжать историю с умной розеткой, а датчик вибраций отложить на потом. Впрочем, пока отключать датчик вибраций тоже не стал, на случай если захочется еще разок покрутить крутилки и уменьшить количество фолзов, правда без особой надежды.
...
Довольно незамысловато и ровно то что нужно. На этом я часть с определением "работает ли стиральная машина" я решил закончить и начать заниматься состоянием дверцы.
...
Осталось дело за малым - подключаемся к питанию и I²C шине:
```markdown
| ESP32          |  VL53L0X  |
|----------------|-----------|
|  3.3V          |    VIN    |
...
На `XSHUT` я забил, т.к. датчик у меня подключается только один и этот pin использоваться не будет. После небольшой пайки и клейки у меня получилась такая михала:
...
И так, к этому моменту я располагал:
  - кастомным событием типа `washer_state` от умной розетки. Состояние может запаздывать на 5+ минут
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/XCrweif3As8?si=4oKOQMWB01iAAzgJ" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
