...
Если говорить предметно о Slate AX, то в интернетах можно найти разные обзоры (e.g. [GL-iNet Slate AX WiFi 6 gigabit wireless travel router review](https://the-gadgeteer.com/2022/09/24/gl-inet-slate-ax-wifi-6-gigabit-wireless-travel-router-review-safety-and-savings-on-the-road/)). Я ни повторяться, ни продавать не хочу, поэтому лучше расскажу, почему я в свое время выбрал его:
  ...
  * есть USB Type-A, который можно использовать для всякой периферии
  - питается от USB Type-C, ненавижу возить лишние и не универсальные зарядки
  - есть поддержка SD карточки, на которую можно закинуть кинчики и стримить с помощью DLNA на телефон/ноут/chromecast/etc
  ...
  - [до недавних событий был достаточно открытым](https://forum.gl-inet.com/t/formal-statement-on-gl-inet-software-going-closed-source/33288)
...
Получается, все чего я должен хотеть - пошифрованный extroot, который будет хранить мои боевые конфиги, ключики, пароли и прочие непотребства. Безопаснее с этим роутером уже не будет. Бонусом появляется возможность загрузиться без использования оного, как-будто так и нужно ;)
...
Тем более, что в моем случае Slate AX оборудован 128MB NAND Flash, из которой пользователю доступно меньше половины:
```
...
52452
```
Это не дофига, но для типичных роутерных нужд хватит и в пару раз меньшего объема. 

Осталось определиться с тем, что расшифрует данные. Я, честно говоря, метался между напилить че-нить самому (закончить-таки Lupa/начать писать Hang) или взять что-то существующее. Любопытство во мне вовремя победило, и я решил воспользоваться FIDO2 токеном. Благо, большая часть реализаций должна поддерживать [HMAC Secret Extension](https://fidoalliance.org/specs/fido-v2.0-id-20180227/fido-client-to-authenticator-protocol-v2.0-id-20180227.html#sctn-hmac-secret-extension):
...
Затем варим контейнер LUKS, поглядывая в гайд [Disk Encryption](https://openwrt.org/docs/guide-user/storage/disk.encryption) от OpenWrt. В качестве контейнера будет использоваться файлик, т.к. 1) это удобнее 2) cryptsetup не поддерживает MTD.
...
Вот и самая интересная часть - подготовка и монтирование extroot. Тут стоит обратиться к гайдам [extroot](https://openwrt.org/docs/guide-user/additional-software/extroot_configuration) и [LUKS encrypted extroot](https://openwrt.org/docs/guide-user/additional-software/extroot_configuration#luks_encrypted_extroot) от OpenWrt. Если в двух словах, наш план таков:
...
Не станем откладывать, форматируем наш раздел:
```
. /etc/fidele.conf
...
FIDO2 токен должен попросить себя потрогать и `/dev/dm-0` смонтироваться в `/overlay`:
```
...
# In action
Самое время проверить делом, я этого долго ждал :) Для наглядности я завел две AP - `GoodPotato` и `BadPotato`. Одна в пошифрованном extroot, другая нет. 

Вот загрузка с разблокировкой LUKS-контейнера:

![](/assets/images/posts/glinet-fido2/good-boot.png)

И без:

![](/assets/images/posts/glinet-fido2/bad-boot.png)
